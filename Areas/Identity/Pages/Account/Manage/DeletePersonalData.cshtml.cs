...
namespace Gra_przegladarkowa.Areas.Identity.Pages.Account.Manage
{
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Wystąpił błąd podczas usuwania użytkownika o ID '{userId}'.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("Użytkownik o ID '{UserId}' usunął się.", userId);
